<?php declare(strict_types=1);

/**
 * Copyright (C) BaseCode Oy - All Rights Reserved
 *
 * For the full copyright and license information, please view
 * the LICENSE file that was distributed with this source code.
 */

namespace BaseCodeOy\Basket\Jurisdictions\SouthAmerica;

use BaseCodeOy\Basket\Contracts\Jurisdiction;
use BaseCodeOy\Basket\Contracts\TaxRate;
use BaseCodeOy\Basket\TaxRates\SouthAmerica\VenezuelaValueAddedTax;
use Money\Currency;

final readonly class Venezuela implements Jurisdiction
{
    private Currency $currency;

    private VenezuelaValueAddedTax $venezuelaValueAddedTax;

    /**
     * Venezuela constructor.
     */
    public function __construct()
    {
        $this->venezuelaValueAddedTax = new VenezuelaValueAddedTax();
        $this->currency = new Currency('VEF');
    }

    /**
     * @return VenezuelaValueAddedTax
     */
    #[\Override()]
    public function rate(): TaxRate
    {
        return $this->venezuelaValueAddedTax;
    }

    #[\Override()]
    public function currency(): Currency
    {
        return $this->currency;
    }
}
